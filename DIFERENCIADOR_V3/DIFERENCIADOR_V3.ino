...
Intervalo iVarBME    = { 22.00,  60.00,  67.00,   96.00 };
Intervalo iArea135   = {  2.40,   3.95,   4.15,    7.60 };
Intervalo iVar135    = { 22.00,  70.00,  95.00,  265.00 };
...
// Si la zona ganadora junta menos que esto, se avisa que
// las variables estan dispersas (pero igual se contesta)
const float CONFIANZA_MINIMA = 0.50;
...
void setup() {
  ...
  Serial.println(F(" v5 - decision por intervalos"));
  Serial.println(F("==================================="));
...
Zona decidirFinal() {
  ...
  if (confianza < CONFIANZA_MINIMA) avisoDispersion = true;
...
void imprimirRecta() {
  ...
  Serial.print(F(" %  ("));
  Serial.print(pesoZona[resultado]);
  Serial.print(F(" / "));
